...
function listdpl {
	while read i; do
		VERSION=$(git show -s --format=%N $i | grep -E "uat|stage|prod")
		if [ ! -z "$VERSION" ]; then
			echo ${VERSION} $i
		fi
...
function printusage {
	printf "\nUsage $0 [uat|stage|prod|reset|list] [version-tag]\n"
	echo "'uat', 'stage' and 'prod' are the only valid environment keywords"
